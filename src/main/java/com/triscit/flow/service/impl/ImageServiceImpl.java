...
public class ImageServiceImpl implements ImageService {
    ...
    public List<String> getHighLightedFlows(BpmnModel bpmnModel,
        ...
        for (HistoricActivityInstance historicActivityInstance : historicActivityInstanceList) {
            // 获取流程节点
            FlowElement flowNode = bpmnModel.getMainProcess().getFlowElement(historicActivityInstance
                    .getActivityId(), true);
            if (flowNode != null && flowNode instanceof FlowNode) {
                allHistoricActivityNodeList.add((FlowNode) flowNode);
                // 结束时间不为空，当前节点则已经完成
    ...
    public byte[] generateImageByProcInstId(String procInstId) throws Exception {
        ...
        try {
            ...
            // 定义流程画布生成器
            ProcessDiagramGenerator processDiagramGenerator = new CustomImageGenerator();
...
//            ProcessDiagramGenerator diagramGenerator = engconf.getProcessDiagramGenerator();
            ...
            imageStream = ((CustomImageGenerator) processDiagramGenerator)
                    .generateDiagramCustom(bpmnModel, "png",
                            highLightedActivitiIdList, runningActivitiIdList,
                            highLightedFlowIds, "宋体",
                            "微软雅黑", "黑体",
                            null, 2.0);
